...
def mobile(request, data=None):
    if data == None:
        mobiles = Product.objects.filter(category='M')
    elif data == 'Redmi' or data == 'Vivo' or data == 'Moto':
    ...
    return render(request, 'app/mobile.html', {'mobiles': mobiles , 'active': 'btn-primary' , 'product_list':product_list})


def laptop(request, data=None):
    if data == None:
        laptops = Product.objects.filter(category='L')
    elif data == 'Acer' or data == 'Microsoft' or data == 'Hp':
...
def topwear(request, data=None):
    if data == None:
        topwears = Product.objects.filter(category='TW')
    elif data == 'Locomotive' or data == 'Zara' or data == 'Denim':
        topwears = Product.objects.filter(category='TW').filter(brand=data)
    elif data == 'below':
        topwears = Product.objects.filter(category='TW').filter(discounted_price__lt=555)
    elif data == 'above':
        topwears = Product.objects.filter(category='TW').filter(discounted_price__gt=550)
...
def bottomwear(request, data=None):
    if data == None:
        bottomwears = Product.objects.filter(category='BW')
    elif data == 'Spyder' or data == 'Zara' or data == 'Denim' or data == 'Lee':
...
class CustomerregistrationView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            messages.success(request, 'Registartion Sucessfull!! , You can Login Now..')
            form.save()
        return render(request, 'app/customerregistration.html',
                      {'form': form})
